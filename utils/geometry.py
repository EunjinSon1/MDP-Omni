...
def applyTransform_volume(transform: np.ndarray, P):
    B = P.shape[0]
    R, tr = getRot(transform), getTr(transform)
    if type(P) == torch.Tensor:
        R = torch.as_tensor(R, device=P.device, dtype=P.dtype)
        tr = torch.as_tensor(tr, device=P.device, dtype=P.dtype)
        return torch.einsum('ij,bjnh->binh', R, P) + tr.view(1, 3, 1, 1)
    else:
        return R.dot(P) + tr
