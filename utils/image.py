...
def imrescale(image: np.ndarray, scale: float) -> np.ndarray:
    image = toNumpy(image)
    dtype = image.dtype
    out = skimage.transform.rescale(image, scale, channel_axis=-1, preserve_range=True)
...
def pixelToGrid_volume(pts, target_resolution: (int, int), 
                source_resolution: (int, int)):
    B,_,N,_ = pts.shape 
    h, w = target_resolution
    height, width = source_resolution
    xs = (pts[:,0,:]) / (width - 1) * 2 - 1
    ys = (pts[:,1,:]) / (height - 1) * 2 - 1
    xs = xs.reshape((B, N, h, w, 1))
    ys = ys.reshape((B, N, h, w, 1))
    return concat((xs, ys), -1)
...
def normalizeImage(image: np.ndarray, mask=None,
    ...
    def __normalizeImage1D(image, mask):
        image = image.squeeze().astype(np.float32) / 255.
