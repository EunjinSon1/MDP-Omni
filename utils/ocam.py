...
class OcamModel:
    ...
    def rayToPixel_volume(self, P, out_theta=False, max_theta=None):
        ...
        x2 = x2.reshape((B,1,-1)) 
        y2 = y2.reshape((B,1,-1)) 
        out = concat((y2, x2), axis=1) 
        out = out.view(B,-1,N,HW)
        out[theta.unsqueeze(1).expand_as(out) > max_theta] = -1e5
        if out_theta:
            return out, theta.float()
        else:
            return out
    ...
    def makeInvisibleMask(self) -> np.ndarray:
        ...
        return invisible.reshape((self.height, self.width)).astype(np.bool_)
